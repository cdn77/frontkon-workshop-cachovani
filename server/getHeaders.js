 ...
 *    - stale-while-revalidate=N  - server stale content (for up to N seconds) and refetch in the background - the next load will serve the newly fetched content from the updated cache
 */
...
export const getHeaders = (path) => {
  ...
  if (path.startsWith("/assets/")) {
    ...
    return {
      "Cache-Control": `public, s-maxage=${twoYears}, max-age=${fiveMinutes}`,
    };
  ...
  return {
    "Cache-Control": `public, s-maxage=${twoYears}, max-age=5, must-revalidate`,
  };
};
